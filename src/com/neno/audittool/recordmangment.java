...
public class recordmangment extends Activity {
	...
	public static ArrayList<Policy_Info> record_info = new ArrayList<Policy_Info>();
	...
	private void addvalues() {
		...
		for (int i = 0; i < policies_question.length; i++) {
			...
			record_info.add(Increament, policyinfo);
		}
	...
	public void OnFarwordClick(View v) {
		...
		if (checkfields()) {
			...
			if (Increament > policies_question.length - 1) {
				...
				checkresult();
				Intent i = new Intent(recordmangment.this,
						record_mangment_scorcard.class);
				i.putExtra("res", Average);
				i.putExtra("data", record_info);
				startActivity(i);
			}
			diplay();
		}
	...
	private void diplay() {
		...
		if (record_info.get(Increament).getDate().equals("")) {

			setdate.setText("");
			Date = "";

		} else {
			Date = record_info.get(Increament).getDate();
			setdate.setText(record_info.get(Increament).getDate());
		}

		if (record_info.get(Increament).getComplaint().equals("")) {

			sp_complaints.setSelection(0);
			sp_evidence
					.setAdapter(new ArrayAdapter<String>(recordmangment.this,
							R.layout.spinner_item, Utils.Evidence2));
		} else {
			if (record_info.get(Increament).getComplaint().equals("NO")) {
				sp_complaints.setSelection(1);
				sp_evidence.setAdapter(new ArrayAdapter<String>(
						recordmangment.this, R.layout.spinner_item,
						Utils.Evidence));
			} else {
				sp_complaints.setSelection(0);
				sp_evidence.setAdapter(new ArrayAdapter<String>(
		...
		if (record_info.get(Increament).getFindings().equals("")) {
			findings.setText("");
		} else {
			findings.setText(record_info.get(Increament).getFindings());
		}
		if (record_info.get(Increament).getEvidence().equals("")) {

			updateimage(0);
		} else {

			updateimage(Integer.parseInt(record_info.get(Increament)
					.getEvidence()) - 1);
		...
		if (record_info.get(Increament).getEvidence().equals("")) {
			...
			if (record_info.get(Increament).getComplaint().equals("NO")) {
				sp_evidence.setSelection(Integer.parseInt(record_info.get(
	...
	private void checkresult() {
		// TODO Auto-generated method stub
		int total = 0;
		for (int k = 0; k < record_info.size(); k++) {
			total += Integer.parseInt(record_info.get(k).getEvidence());
			if (record_info.get(k).getComplaint().equalsIgnoreCase("YES")) {
		...
		Average = total / record_info.size();
