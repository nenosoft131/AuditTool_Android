...
public class managmentreview extends Activity {
	...
	public static ArrayList<Policy_Info> managmengrev_info = new ArrayList<Policy_Info>();
	...
	private void addvalues() {
		...
		for (int i = 0; i < policies_question.length; i++) {
			...
			managmengrev_info.add(Increament, policyinfo);
		}
	...
	public void OnFarwordClick(View v) {
		...
		if (checkfields()) {
			...
			if (Increament > policies_question.length - 1) {
				...
				checkresult();
				Intent i = new Intent(managmentreview.this,
						mangment_review_scorcard.class);
				i.putExtra("res", Average);
				i.putExtra("data", managmengrev_info);
				startActivity(i);
			}
			diplay();
		}
	...
	private void diplay() {
		...
		if (managmengrev_info.get(Increament).getDate().equals("")) {

			setdate.setText("");
			Date="";

		} else {
			Date = managmengrev_info.get(Increament).getDate();
			setdate.setText(managmengrev_info.get(Increament).getDate());
		}

		if (managmengrev_info.get(Increament).getComplaint().equals("")) {

			sp_complaints.setSelection(0);
			sp_evidence
					.setAdapter(new ArrayAdapter<String>(managmentreview.this,
							R.layout.spinner_item, Utils.Evidence2));
		} else {
			if (managmengrev_info.get(Increament).getComplaint().equals("NO")) {
				sp_complaints.setSelection(1);
				sp_evidence.setAdapter(new ArrayAdapter<String>(
						managmentreview.this, R.layout.spinner_item,
						Utils.Evidence));
			} else {
				sp_complaints.setSelection(0);
				sp_evidence.setAdapter(new ArrayAdapter<String>(
		...
		if (managmengrev_info.get(Increament).getFindings().equals("")) {
			findings.setText("");
		} else {
			findings.setText(managmengrev_info.get(Increament).getFindings());
		}
		if (managmengrev_info.get(Increament).getEvidence().equals("")) {

			updateimage(0);
		} else {

			updateimage(Integer.parseInt(managmengrev_info.get(Increament)
					.getEvidence()) - 1);
		...
		if (managmengrev_info.get(Increament).getEvidence().equals("")) {
			...
			if (managmengrev_info.get(Increament).getComplaint().equals("NO")) {
				sp_evidence.setSelection(Integer.parseInt(managmengrev_info.get(
	...
	private void checkresult() {
		// TODO Auto-generated method stub
		int total = 0;
		for (int k = 0; k < managmengrev_info.size(); k++) {
			total += Integer.parseInt(managmengrev_info.get(k).getEvidence());
			if (managmengrev_info.get(k).getComplaint().equalsIgnoreCase("YES")) {
		...
		Average = total / managmengrev_info.size();
	...
	private void savedata() {
		...
		managmengrev_info.remove(Increament);
		managmengrev_info.add(Increament, policyinfo);
