...
public class policy extends Activity {
	...
	public static ArrayList<Policy_Info> poicy_info = new ArrayList<Policy_Info>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		reationals = getResources().getStringArray(R.array.policy_rationale);
	...
	private void addvalues() {
		...
		for (int i = 0; i < 4; i++) {
			...
			poicy_info.add(Increament, policyinfo);
		}
	...
	public void OnFarwordClick(View v) {
		...
		if (checkfields()) {
			...
			if (Increament > 3) {
				Increament = 3;
				checkresult();
				Intent i = new Intent(policy.this, policyscrocard.class);
				i.putExtra("res", Average);
				i.putExtra("data", poicy_info);
				startActivity(i);
			}
			diplay();
		}
	...
	private void diplay() {
		...
		if (poicy_info.get(Increament).getDate().equals("")) {

			setdate.setText("");
			Date = "";

		} else {
			Date = poicy_info.get(Increament).getDate();
			setdate.setText(poicy_info.get(Increament).getDate());

		}

		if (poicy_info.get(Increament).getComplaint().equals("")) {

			sp_complaints.setSelection(0);
			sp_evidence.setAdapter(new ArrayAdapter<String>(policy.this,
					R.layout.spinner_item, Utils.Evidence2));
		} else {
			if (poicy_info.get(Increament).getComplaint().equals("NO")) {
		...
		if (poicy_info.get(Increament).getFindings().equals("")) {
			findings.setText("");
		} else {
			findings.setText(poicy_info.get(Increament).getFindings());
		}
		...
		if (poicy_info.get(Increament).getEvidence().equals("")) {

			updateimage(0);
		} else {

			updateimage(Integer.parseInt(poicy_info.get(Increament)
					.getEvidence()) - 1);
		...
		if (poicy_info.get(Increament).getEvidence().equals("")) {
			...
			if (poicy_info.get(Increament).getComplaint().equals("NO")) {
				sp_evidence.setSelection(Integer.parseInt(poicy_info.get(
				...
				if (check) {
					...
					if (Utils.COMPLIANT[sp_complaints.getSelectedItemPosition()]
						...
						sp_evidence.setAdapter(new ArrayAdapter<String>(
								policy.this, R.layout.spinner_item,
								Utils.Evidence2));
					} else {
						sp_evidence.setAdapter(new ArrayAdapter<String>(
								policy.this, R.layout.spinner_item,
	...
	private void checkresult() {
		// TODO Auto-generated method stub
		int total = 0;
		for (int k = 0; k < poicy_info.size(); k++) {
			total += Integer.parseInt(poicy_info.get(k).getEvidence());
			if (poicy_info.get(k).getComplaint().equalsIgnoreCase("YES")) {
