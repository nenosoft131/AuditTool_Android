...
public class whsmaudit extends Activity {
	...
	public static ArrayList<Policy_Info> whsm_info = new ArrayList<Policy_Info>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		policies_question = getResources().getStringArray(R.array.whsms_audit);
	...
	private void addvalues() {
		...
		for (int i = 0; i < policies_question.length; i++) {
			...
			whsm_info.add(Increament, policyinfo);
		}
	...
	public void OnFarwordClick(View v) {
		...
		if (checkfields()) {
			...
			if (Increament > policies_question.length - 1) {
				...
				checkresult();
				Intent i = new Intent(whsmaudit.this, whsms_scorcard.class);
				i.putExtra("res", Average);
				i.putExtra("data", whsm_info);
				startActivity(i);
			}
			diplay();
		}
	...
	private void diplay() {
		...
		if (whsm_info.get(Increament).getDate().equals("")) {

			setdate.setText("");
			Date = "";

		} else {
			Date = whsm_info.get(Increament).getDate();
			setdate.setText(whsm_info.get(Increament).getDate());
		}

		if (whsm_info.get(Increament).getComplaint().equals("")) {

			sp_complaints.setSelection(0);
			sp_evidence.setAdapter(new ArrayAdapter<String>(whsmaudit.this,
					R.layout.spinner_item, Utils.Evidence2));
		} else {
			if (whsm_info.get(Increament).getComplaint().equals("NO")) {
		...
		if (whsm_info.get(Increament).getFindings().equals("")) {
			findings.setText("");
		} else {
			findings.setText(whsm_info.get(Increament).getFindings());
		}
		if (whsm_info.get(Increament).getEvidence().equals("")) {

			updateimage(0);
		} else {

			updateimage(Integer.parseInt(whsm_info.get(Increament)
					.getEvidence()) - 1);
		...
		if (whsm_info.get(Increament).getEvidence().equals("")) {
			...
			if (whsm_info.get(Increament).getComplaint().equals("NO")) {
				sp_evidence.setSelection(Integer.parseInt(whsm_info.get(
				...
				if (check) {
					...
					if (Utils.COMPLIANT[sp_complaints.getSelectedItemPosition()]
						...
						sp_evidence.setAdapter(new ArrayAdapter<String>(
								whsmaudit.this, R.layout.spinner_item,
								Utils.Evidence2));
					} else {
						sp_evidence.setAdapter(new ArrayAdapter<String>(
								whsmaudit.this, R.layout.spinner_item,
	...
	private void checkresult() {
		// TODO Auto-generated method stub
		int total = 0;
		for (int k = 0; k < whsm_info.size(); k++) {
			total += Integer.parseInt(whsm_info.get(k).getEvidence());
			if (whsm_info.get(k).getComplaint().equalsIgnoreCase("YES")) {
