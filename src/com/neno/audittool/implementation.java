...
public class implementation extends Activity {
	...
	public static ArrayList<Policy_Info> implementation_info = new ArrayList<Policy_Info>();
	...
	private void addvalues() {
		...
		for (int i = 0; i < policies_question.length; i++) {
			...
			implementation_info.add(Increament, policyinfo);
		}
	...
	public void OnFarwordClick(View v) {
		...
		if (checkfields()) {
			...
			if (Increament > policies_question.length - 1) {
				...
				checkresult();
				Intent i = new Intent(implementation.this,
						Implementationscrocard.class);
				i.putExtra("res", Average);
				i.putExtra("data", implementation_info);
				startActivity(i);
			}
			diplay();
		}
	...
	private void diplay() {
		...
		if (implementation_info.get(Increament).getDate().equals("")) {

			setdate.setText("");
			Date="";

		} else {
			Date = implementation_info.get(Increament).getDate();
			setdate.setText(implementation_info.get(Increament).getDate());
		}

		if (implementation_info.get(Increament).getComplaint().equals("")) {

			sp_complaints.setSelection(0);
			sp_evidence
					.setAdapter(new ArrayAdapter<String>(implementation.this,
							R.layout.spinner_item, Utils.Evidence2));
		} else {
			if (implementation_info.get(Increament).getComplaint().equals("NO")) {
				sp_complaints.setSelection(1);
				sp_evidence.setAdapter(new ArrayAdapter<String>(
						implementation.this, R.layout.spinner_item,
						Utils.Evidence));
			} else {
				sp_complaints.setSelection(0);
				sp_evidence.setAdapter(new ArrayAdapter<String>(
		...
		if (implementation_info.get(Increament).getFindings().equals("")) {
			findings.setText("");
		} else {
			findings.setText(implementation_info.get(Increament).getFindings());
		}
		if (implementation_info.get(Increament).getEvidence().equals("")) {

			updateimage(0);
		} else {

			updateimage(Integer.parseInt(implementation_info.get(Increament)
					.getEvidence()) - 1);
		...
		if (implementation_info.get(Increament).getEvidence().equals("")) {
			...
			if (implementation_info.get(Increament).getComplaint().equals("NO")) {
				sp_evidence.setSelection(Integer.parseInt(implementation_info.get(
	...
	private void checkresult() {
		// TODO Auto-generated method stub
		int total = 0;
		for (int k = 0; k < implementation_info.size(); k++) {
			total += Integer.parseInt(implementation_info.get(k).getEvidence());
			if (implementation_info.get(k).getComplaint().equalsIgnoreCase("YES")) {
		...
		Average = total / implementation_info.size();
	...
	private void savedata() {
		...
		implementation_info.remove(Increament);
		implementation_info.add(Increament, policyinfo);
