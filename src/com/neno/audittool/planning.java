...
public class planning extends Activity {
	...
	ImageView im;
	TextView question, top,rational_tv;
	EditText findings;
	LinearLayout rl;
	Typeface myfont;
	private int myYear, myMonth, myDay;
	static final int ID_DATEPICKER = 3;
	String[] policies_question,reationals;
	int count = 0;
	...
	public static ArrayList<Policy_Info> planning_info = new ArrayList<Policy_Info>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		reationals = getResources().getStringArray(
	...
	private void addvalues() {
		...
		for (int i = 0; i < 15; i++) {
			...
			planning_info.add(Increament, policyinfo);
		}
	...
	public void OnFarwordClick(View v) {
		...
		if (checkfields()) {
			...
			if (Increament > 14) {
				Increament = 14;
				checkresult();
				Intent i = new Intent(planning.this, planningscrocard.class);
				i.putExtra("res", Average);
				i.putExtra("data", planning_info);
				startActivity(i);
			}
			diplay();
		}
	...
	private void diplay() {
		...
		if (planning_info.get(Increament).getDate().equals("")) {

			setdate.setText("");
			Date="";

		} else {
			Date = planning_info.get(Increament).getDate();
			setdate.setText(planning_info.get(Increament).getDate());
		}

		if (planning_info.get(Increament).getComplaint().equals("")) {

			sp_complaints.setSelection(0);
			sp_evidence.setAdapter(new ArrayAdapter<String>(planning.this,
					R.layout.spinner_item, Utils.Evidence2));
		} else {
			if (planning_info.get(Increament).getComplaint().equals("NO")) {
		...
		if (planning_info.get(Increament).getFindings().equals("")) {
			findings.setText("");
		} else {
			findings.setText(planning_info.get(Increament).getFindings());
		}
		
		if(reationals[Increament].length()>0)
		{
			rl.setVisibility(View.VISIBLE);
			rational_tv.setText(reationals[Increament]);
			
		}
		else{
			rl.setVisibility(View.INVISIBLE);
		}
		if (planning_info.get(Increament).getEvidence().equals("")) {

			updateimage(0);
		} else {

			updateimage(Integer.parseInt(planning_info.get(Increament)
					.getEvidence()) - 1);
		...
		if (planning_info.get(Increament).getEvidence().equals("")) {
			...
			if (planning_info.get(Increament).getComplaint().equals("NO")) {
				sp_evidence.setSelection(Integer.parseInt(planning_info.get(
	...
	private void init() {
		...
		rl=(LinearLayout) findViewById(R.id.rational__ll);
		rational_tv=(TextView) findViewById(R.id.tv_rational);
		rational_tv.setTypeface(myfont);
				...
				if (check) {
					...
					if (Utils.COMPLIANT[sp_complaints.getSelectedItemPosition()]
						...
						sp_evidence.setAdapter(new ArrayAdapter<String>(
								planning.this, R.layout.spinner_item,
								Utils.Evidence2));
					} else {
						sp_evidence.setAdapter(new ArrayAdapter<String>(
								planning.this, R.layout.spinner_item,
	...
	private void initvalue() {
		...
		rl.setVisibility(View.VISIBLE);
		rational_tv.setText(reationals[count]);
		findings.setText("");
	...
	private void checkresult() {
		// TODO Auto-generated method stub
		int total = 0;
		for (int k = 0; k < planning_info.size(); k++) {
			total += Integer.parseInt(planning_info.get(k).getEvidence());
			if (planning_info.get(k).getComplaint().equalsIgnoreCase("YES")) {
		...
		Average = total / planning_info.size();
	...
	private void savedata() {
		...
		planning_info.remove(Increament);
		planning_info.add(Increament, policyinfo);
